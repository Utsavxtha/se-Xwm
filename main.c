...
#define TOTALKEYS 26
char keyBindings[TOTALKEYS][2] = {"Z", "D", "M", "J", "K", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "H", "L", "T", "C", "O", "P", "Y", "I", "W", "R", "B"};
...
Colormap cmap = DefaultColormap(display, DefaultScreen(display));
  ...
  for (int i = 0; i < pertag_win[working_tag]; i++)
  {
    ...
    { // this means it is the master window
      ...
      if (total_windows_in_this_tag == 1)
      { // take the full size if it is the only window
        XResizeWindow(display, working, width- border_pixel, height-border_pixel);
      }
      else
      {
